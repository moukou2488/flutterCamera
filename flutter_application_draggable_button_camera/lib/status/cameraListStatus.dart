...
class CameraListStatusSelected extends CameraListStatus {
  ...
  CameraListStatusSelected(
      ...
      required this.customResolution // 해상도
      });

  CameraDescription get actual => cameras[indexSelected];
  ResolutionPreset get resolution => customResolution.resolutionPreset;
  CustomResolution get customResolutions =>
      customResolution; // TODO :: get으로 안하고 변수로 접근해서 꺼내면 에러가 난다 왤까..? 찾아보기
}
...
extension CameraListStatusExt on CameraListStatus {
  ...
  dynamic when({
    ...
    dynamic Function(
            CameraDescription camera, CustomResolution customResolution)?
        selected,
      ...
      case CameraListStatusSelected:
        {
          if (selected != null) {
            return selected(
                this.selected.actual, this.selected.customResolutions);
          } else {
            return orElse();
          }
        }
